		...
		try {
			...
			if (timeZone != null) {
				if (timeZone.getDSTSavings() > 0) {
					time = time - (timeZone.getOffset(time) - timeZone.getDSTSavings());
				}
			}
		} catch (Exception e) {
			//Do nothing
		...
		if (numberOfThisDisk == 0) {
			...
			for (int i = 0; i <= numberOfThisDisk; i++) {
				if (i == numberOfThisDisk) {
					...
					if (i >= 9) {
						partFile = currZipFile.substring(0, currZipFile.lastIndexOf(".")) + ".z" + (i + 1);
					} else {
						partFile = currZipFile.substring(0, currZipFile.lastIndexOf(".")) + ".z0" + (i + 1);
					}
