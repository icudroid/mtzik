...
public class ArchiveMaintainer {
	...
	public HashMap initRemoveZipFile(ZipModel zipModel, 
		...
		File zipFile = null;
		Zip4jInputRAF inputStream = null;
		...
		try {
			...
			try {
				outputStream = new Zip4jOutputStream(new File(tmpZipFileName), InternalZipConstants.WRITE_MODE);
			...
			zipModel.getEndCentralDirRecord().setOffsetOfStartOfCentralDir((int)outputStream.getFilePointer());
	...
	private void copyFile(Zip4jInputRAF inputStream, 
			Zip4jOutputStream outputStream, long start, long end, ProgressMonitor progressMonitor) throws ZipException {
	...
	private Zip4jInputRAF createFileHandler(ZipModel zipModel, String mode) throws ZipException {
		...
		try {
			Zip4jInputRAF raf = null;
			if (zipModel.isNestedZipFile()) {
				raf = new Zip4jInputRAF(new File(zipModel.getZipFile()), mode, 
						zipModel.getStart(), zipModel.getEnd());
			} else {
				raf = new Zip4jInputRAF(new File(zipModel.getZipFile()), mode);
			}
			return raf;
	...
	private void initMergeSplitZipFile(ZipModel zipModel, File outputZipFile, 
		...
		if (zipModel == null) {
			throw new ZipException("one of the input parameters is null, cannot merge split zip files");
		}
		...
		try {
				...
				if (i == 0) {
					if (zipModel.getCentralDirectory() != null && 
						...
						if (Raw.readIntLittleEndian(buff, 0) == InternalZipConstants.SPLITSIG) {
							start = 4;
							((FileHeader)zipModel.getCentralDirectory().getFileHeaders().get(0)).setOffsetLocalHeader(0);
						}
					}
				}
			...
			updateSplitZipModel(newZipModel, fileSizeList);
	...
	private Zip4jInputRAF createSplitZipFileHandler(ZipModel zipModel, int partNumber) throws ZipException {
		if (zipModel == null) {
		...
		try {
			...
			return new Zip4jInputRAF(partFile, InternalZipConstants.READ_MODE);
		} catch (FileNotFoundException e) {
			throw new ZipException(e);
		} catch (Exception e) {
			throw new ZipException(e);
	...
	private Zip4jOutputStream prepareOutputStreamForMerge(File outFile) throws ZipException {
		if (outFile == null) {
			throw new ZipException("outFile is null, cannot create outputstream");
		}
		
		try {
			return new Zip4jOutputStream(outFile, InternalZipConstants.WRITE_MODE);
		} catch (FileNotFoundException e) {
			throw new ZipException(e);
		} catch (Exception e) {
			throw new ZipException(e);
	...
	private void updateSplitZipModel(ZipModel zipModel, ArrayList fileSizeList) throws ZipException {
		if (zipModel == null) {
	...
	private void updateSplitFileHeader(ZipModel zipModel, ArrayList fileSizeList) throws ZipException {
		try {
			...
			for (int i = 0; i < fileHeaderCount; i++) {
				...
				((FileHeader)zipModel.getCentralDirectory().getFileHeaders().get(i)).setOffsetLocalHeader(
						((FileHeader)zipModel.getCentralDirectory().getFileHeaders().get(i)).getOffsetLocalHeader() + offsetLHToAdd);
	...
	public void setComment(ZipModel zipModel, String comment) throws ZipException {
		...
		zipModel.getEndCentralDirRecord().setCommentBytes(comment.getBytes());
		zipModel.getEndCentralDirRecord().setCommentLength(comment.length());
		
		Zip4jOutputStream outputStream = null;
		
		try {
			HeaderWriter headerWriter = new HeaderWriter();
			outputStream = new Zip4jOutputStream(new File(zipModel.getZipFile()), InternalZipConstants.WRITE_MODE);
