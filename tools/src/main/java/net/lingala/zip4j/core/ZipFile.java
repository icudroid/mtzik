...
public class ZipFile {
	...
	public void addFiles(ArrayList sourceFileList, ZipParameters parameters) throws ZipException {
		...
		ZipEngine zipEngine = new ZipEngine(zipModel, isSplitEligible);
	...
	/*
	public ZipModel readNestedZipFile(ZipModel parentZipModel, FileHeader fileHeader) throws ZipException {
		Zip4jInputStream inputStream = null;
		
		try {
			inputStream = new Zip4jInputStream(new File(file), Zip4jConstants.READ_MODE);
			NestedZipFileReader nestedZipFileReader = new NestedZipFileReader(inputStream);
			return nestedZipFileReader.readNestedZipFile(parentZipModel, fileHeader);
		} catch (FileNotFoundException e) {
			throw new ZipException(e);
		} finally {
			if (inputStream != null) {
	...
	public void setFileNameCharset(String charsetName) throws ZipException {
		...
		if (zipModel == null) {
			throw new ZipException("zip model is null, cannot assign charset");
		}
		
		zipModel.setFileNameCharset(charsetName);
	}
...
//	/**
//	 * Returns the zip model for the zip file
//	 * @return ZipModel
//	 */
