...
<form:form commandName="player" method="post" action="playerform.html" onsubmit="return onFormSubmit(this)" id="playerform">
<form:hidden path="id"/>
...
<c:if test="${empty player.version}">
...
<c:choose>
    ...
    <c:when test="${not empty player.username}">
    <li>
        <strong><appfuse:label key="user.roles"/>:</strong>
        <c:forEach var="role" items="${player.roleList}" varStatus="status">
...
<script type="text/javascript">
    Form.focusFirstElement($('playerForm'));
    highlightFormElements();

    function passwordChanged(passwordField) {
        if (passwordField.id == "password") {
            var origPassword = "${player.password}";
        } else if (passwordField.id == "confirmPassword") {
            var origPassword = "${player.confirmPassword}";
        }
